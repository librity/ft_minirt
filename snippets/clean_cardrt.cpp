...
/*   Updated: 2021/04/04 22:51:03 by lpaulo-m         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int trace(vector_3d o, vector_3d d, float &t, vector_3d &n)
{
	t = 1e9;
	int m = 0;
	float p = -o.z / d.z;
...
vector_3d S(vector_3d o, vector_3d d)
{
	...
	vector_3d h = o + d * t, l = !(vector_3d(9 + random(), 9 + random(), 16) + h * -1), r = d + n * (n % d * -2);
	float b = l % n;
...
int main()
{
	...
	for (int y = 512; y--;)
	{
		for (int x = 512; x--;)
		{
			...
			for (int r = 64; r--;)
			{
				vector_3d t = a * (random() - .5) * 99 + b * (random() - .5) * 99;
				p = S(vector_3d(17, 16, 8) + t, !(t * -1 + (a * (random() + x) + b * (y + random()) + c) * 16)) * 3.5 + p;
			}
